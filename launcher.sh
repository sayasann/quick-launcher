...
if [[ "$menu" == "1" ]]; then
    ...
    echo ""
    echo "========================================="
    echo "    UYGULAMALARI KAPAT"
    echo "========================================="
    echo ""
    while IFS= read -r app; do
        [ -z "$app" ] && continue
        name=$(basename "$app" | sed 's/\.exe$//')
        echo "Kapatiliyor: $name"
        powershell.exe -Command "Stop-Process -Name '$name' -Force -ErrorAction SilentlyContinue"
        echo "OK"
    done < "$APPS_FILE"
    echo ""
    echo "Tum uygulamalar kapatildi!"
